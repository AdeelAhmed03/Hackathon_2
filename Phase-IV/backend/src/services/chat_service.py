...
def get_response_text(response: Any) -> str:
    """
    Extract text from Cohere response object.

    In SDK v5.x, text is in response.message.content[0].text
...
def get_response_tool_calls(response: Any) -> Optional[List[Any]]:
    """
    Extract tool calls from Cohere response object.

    In SDK v5.x, tool_calls are in response.message.tool_calls
